...
:root{
  /* Layout widths */
  --sb-width: 232px;        /* expanded width of sidebar */
  --sb-collapsed: 72px;     /* collapsed width (icons only rail) */

  /* Shared width for nav pills & bottom buttons (expanded state) */
  --item-max-width: 80%;    /* e.g. 90%, 85%, 80% of sidebar width */
  ...
  --sb-muted: #6b7280;
  --sb-border: #e5e7eb;
  --sb-hover: #eef2ff;      /* active/hover pill color */
  --sb-brand-pill: #eef2ff; /* brand logo chip */
  --sb-accent: #2563eb;

  /* Effects */
  --sb-shadow: 0 6px 18px rgba(31,41,55,.08);
  --sb-radius: 12px;        /* pill/button corner radius */
  --sb-icon-size: 18px;
  --sb-z: 1000;

  /* Buttons */
  --btn-padding: 12px;      /* inner padding for Pin/Logout */
  --btn-gap: 10px;          /* spacing between bottom buttons */
  --btn-radius: 12px;
}
...
body.kts-has-sidebar{
  ...
  background:#f5f7fb; color:var(--sb-text);
  font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,"Apple Color Emoji","Segoe UI Emoji";
}
body.kts-sidebar-expanded{ margin-left: var(--sb-width); }
...
.kts-sidebar:hover{ width: var(--sb-width); }
body.kts-sidebar-expanded .kts-sidebar{ width: var(--sb-width); }
...
.kts-logo{
  ...
  background:var(--sb-brand-pill); color:var(--sb-accent); font-weight:800;
}
.kts-title{ font-weight:800; letter-spacing:.2px; white-space:nowrap; display:none; }
.kts-sidebar:hover .kts-title,
body.kts-sidebar-expanded .kts-title{ display:inline; }
...
/* =================== Navigation =================== */
.kts-nav{ list-style:none; margin:6px 0 0; padding:0; }
.kts-nav li{ margin: 2px 0; }
...
.kts-link:hover{ background: var(--sb-hover); }
.kts-link.active{ background: var(--sb-hover); font-weight:700; }
.kts-icon{
  width:28px; min-width:28px; height:28px;
  display:inline-flex; align-items:center; justify-content:center;
  font-size: var(--sb-icon-size);
}
.kts-label{ white-space:nowrap; overflow:hidden; text-overflow:ellipsis; display:none; }
.kts-sidebar:hover .kts-label,
body.kts-sidebar-expanded .kts-label{ display:inline; }
...
body.kts-has-sidebar:not(.kts-sidebar-expanded) .kts-sidebar:not(:hover) .kts-link{
  ...
  gap:0;                     /* no gap without label */
  padding-left:0; padding-right:0; /* remove side padding for visual centering */
  max-width: calc(var(--sb-collapsed) - 8px); /* pill fits the rail with a small inset */
}

/* =================== Bottom Controls =================== */
.kts-bottom{ display:grid; gap:var(--btn-gap); margin-top:8px; }
...
.kts-btn:hover, .kts-logout:hover{ background: var(--sb-hover); }

/* Hide labels in collapsed (show on hover/pin) */
.kts-btn .kts-label, .kts-logout .kts-label{ display:none; }
.kts-sidebar:hover .kts-btn .kts-label, .kts-sidebar:hover .kts-logout .kts-label,
body.kts-sidebar-expanded .kts-btn .kts-label, body.kts-sidebar-expanded .kts-logout .kts-label{ display:inline; }
...
@media (max-width: 860px){
  ...
  body.kts-has-sidebar, body.kts-sidebar-expanded { margin-left:0; }
  .kts-sidebar{
    position:sticky; top:0; width:100%; height:auto;
    border-right:0; border-bottom:1px solid var(--sb-border);
    grid-template-rows:auto auto auto;
  }
  .kts-title, .kts-label{ display:inline !important; }
  .kts-brand, .kts-link, .kts-btn, .kts-logout{ max-width: 100%; }
}
</style>

<aside class="kts-sidebar" id="ktsSidebar" aria-label="Primary sidebar">
  <!-- Brand -->
  <a class="kts-brand" href="<?php echo $BASE; ?>/Dashboard/dashboard.php" title="Dashboard">
  ...
  <!-- Navigation -->
  <ul class="kts-nav">
    <li><a class="kts-link <?php echo kts_active('/Dashboard/dashboard.php',$currentUri); ?>" href="<?php echo $BASE; ?>/Dashboard/dashboard.php" title="Dashboard"><span class="kts-icon">📊</span><span class="kts-label">Dashboard</span></a></li>
    <li><a class="kts-link <?php echo kts_active('/Dashboard/profile/',$currentUri); ?>" href="<?php echo $BASE; ?>/Dashboard/profile/profile.php" title="Profile"><span class="kts-icon">👤</span><span class="kts-label">Profile</span></a></li>
    <li><a class="kts-link <?php echo kts_active('/Dashboard/fields/',$currentUri); ?>" href="<?php echo $BASE; ?>/Dashboard/fields/fields.php" title="Fields"><span class="kts-icon">🌱</span><span class="kts-label">Fields</span></a></li>
